...
# V6 macOS Development Environment Installer
...
install_android_sdk() {
    if prompt_install "Android SDK Command-line Tools"; then
        ...
        if ! unzip -q "$SDK_ZIP" -d ~/Library/Android/sdk; then
...
main() {
    print_color "GREEN" "Starting V6 macOS Development Environment Installation"
...
if [ -t 0 ]; then
    ...
    print_color "RED" "This script must be run interactively. Please download it and run it directly, not through curl | bash."
    exit 1
fi
